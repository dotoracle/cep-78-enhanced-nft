...
const main = async () => {
  ...
  const collection_name = new CLString("Apoc NFT");
  // --session-arg "collection_symbol:string=''" \
  const collection_symbol = new CLString("APOC");
  // --session-arg "total_token_supply:u64='10'" \
  const total_token_supply = new CLU64("1000");
  // --------------------
  // |    Mode      | u8 |
  // --------------------
  // | Minter       | 0  |
  ...
  const metadata_mutability = new CLU8(0); // CAN NOT CHANGE METADATA
  ...
  let deploy = DeployUtil.makeDeploy(
    ...
    DeployUtil.standardPayment(220000000000) // 170 CSPR IS ENOUGH    165000000000
  );
